...
export default function ClickRelationTable() {
  ...
  const API_URL = "http://localhost:8000/api/dashboard/click_relation/";
...
useEffect(() => {
  ...
  const handleRowClick = async (elemento) => {
    ...
    try {
      const url = new URL(
        `http://localhost:8000/api/dashboard/click_detail/${encodeURIComponent(elemento)}/`
      );
...
const handleSessionClick = async (session_id) => {
  ...
  try {
    const url = new URL(`http://localhost:8000/api/dashboard/user_click_flow/`);
